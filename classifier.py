...
class Normalized_Correlation_Layer(Layer):
    ...
    def call(self, x, mask=None):
        ...
        input_1 = K.spatial_2d_padding(
            input_1, padding=(padding_row, padding_col))
        input_2 = K.spatial_2d_padding(input_2, padding=(
            (padding_row[0]*2, padding_row[1]*2), padding_col))
        ...
        for k in range(inp_shape[-1]):
            ...
            for i in range(output_row):
                ...
                slice_row2 = slice(
                    i+padding_row[0], i + self.kernel_size[0]+padding_row[0])
                for j in range(output_col):
            ...
            # batch_size x w'h' x (k**2*d), w': w/subsample-1
            ...
            # batch_size x wh x (k**2*d), w: output_row
            ...
            block = K.reshape(
                block, (-1, output_row, output_col, inp_shape[1]*self.kernel_size[0]))
            output.append(block)
...
def normalized_X_corr_model():
    ...
    b = Input((160, 60, 3))
    model = Sequential()
    model.add(Conv2D(kernel_size=(5, 5), filters=20,
                     input_shape=(160, 60, 3), activation='relu'))
    model.add(MaxPooling2D((2, 2)))
    model.add(Conv2D(kernel_size=(5, 5), filters=25, activation='relu'))
    model.add(MaxPooling2D((2, 2)))
    model1 = model(b)
    model2 = model(a)
    normalized_layer = Normalized_Correlation_Layer(
        stride=(1, 1), patch_size=(5, 5))([model1, model2])
...
def norm_model(input_size=(8, 8, 2)):
    a = Input(input_size)
    b = Input(input_size)
    output = Normalized_Correlation_Layer(
        stride=(1, 1), patch_size=(5, 5))([a, b])
...
if __name__ == "__main__":
    ...
    try:
        ...
        output = test_mod.fit([a, b], l, batch_size=64,
                              shuffle=True, verbose=2, epochs=10)
